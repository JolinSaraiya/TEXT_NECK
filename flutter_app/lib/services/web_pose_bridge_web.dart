...
/// Triggers MediaPipe Pose detection on a static photo.
Future<void> analyzeStaticImage(String dataUrl) async {
  try {
    js.context.callMethod('analyzeStaticImage', [dataUrl]);
    // Wait for JS to finish (up to 8 seconds)
    for (int i = 0; i < 80; i++) {
      await Future.delayed(const Duration(milliseconds: 100));
      final ready = js.context['_staticPoseReady'];
      if (ready == true) return;
    }
  } catch (e) {
    debugPrint('[WebPoseBridge] analyzeStaticImage error: $e');
  }
}

/// Reads the result of the last static image analysis.
WebPoseLandmarks? getStaticPoseResult() {
  final raw = js.context['_staticPoseResult'];
  if (raw == null) return null;
